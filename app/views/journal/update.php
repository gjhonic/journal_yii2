...
$this->title = 'Update Journal: ' . $model->journal_title;
$this->params['breadcrumbs'][] = ['label' => 'Journals', 'url' => ['index']];
$this->params['breadcrumbs'][] = ['label' => $model->journal_title, 'url' => ['view', 'id' => $model->journal_id]];
			...
			<?php $i=0;
				foreach ($Authors as $author):  $i++; ?>
					<tr>
				      ...
				      <td>
                        <?php if($author->isWriterJournal($model->journal_id)){ ?>
                            <input type='checkbox' style='width:20px; border:1px solid grey; height:20px;' name='tp_<?=$author->author_id?>' id='tp_"<?=$author->author_id?>"' checked>
                        <?php }else{ ?>
